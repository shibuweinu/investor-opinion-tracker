...
class FileTrustBackend:
    ...
    def write(self, value: str) -> None:
        ...
        self.path.write_text(value, encoding="utf-8"); os.chmod(self.path, 0o600)
    def delete(self) -> None:
...
class KeychainTrustBackend:
    ...
    def write(self, value: str) -> None:
        subprocess.run(["security", "add-generic-password", "-U", "-s", self.service,
                        "-a", self.account, "-w", value], check=True, capture_output=True)
    def delete(self) -> None:
        subprocess.run(["security", "delete-generic-password", "-s", self.service,
                        "-a", self.account], capture_output=True)


class DeviceTrustStore:
    def __init__(self, backend: TrustBackend): self.backend = backend
    ...
    def is_trusted(self, identity: RepositoryIdentity) -> bool:
        ...
        return all(getattr(record, field) == getattr(identity, field)
                   for field in ("canonical_remote", "owner", "git_identity"))
    def revoke(self) -> None: self.backend.delete()
