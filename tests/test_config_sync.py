...
def document() -> PortableConfig:
    return PortableConfig.model_validate({
        ...
        "reports": {
            "daily": {"lookback_days_by_role": {"research": 5, "auxiliary_news": 2}, "weekdays": [1,2,3,4,5], "hour": 21},
            "weekly": {"lookback_days_by_role": {"research": 7, "auxiliary_news": 7}, "weekdays": [0], "hour": 18},
        },
        ...
        "sync": {"trusted_auto_apply": True}, "updated_at": datetime.now(UTC), "revision": "r1",
    })
...
def test_push_writes_only_allowlisted_files(tmp_path):
    ...
    service.connect(); service.push(document())
    assert {p.name for p in (tmp_path / "checkout").iterdir() if p.name != ".git"} == {"README.md", "config.json", ".gitignore"}
