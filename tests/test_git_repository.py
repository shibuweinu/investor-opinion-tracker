...
def test_rejects_push_when_remote_advanced(tmp_path):
    ...
    first.clone_or_open()
    first.write("config.json", "{}")
    first.commit(["config.json"], "one")
    first.push_fast_forward()
    second = GitRepository(str(remote), tmp_path / "two")
    second.clone_or_open()
    first.write("config.json", '{"v":2}')
    first.commit(["config.json"], "two")
    first.push_fast_forward()
    second.write("README.md", "local")
    second.commit(["README.md"], "local")
