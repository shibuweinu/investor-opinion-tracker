...
定时报告调用 `analyze-file` 时必须传入 `--market-as-of <报告截止时间>`。核验器从日线中选择
不晚于该截止时间的最后一根完整 K 线：09:00 早报通常对应上一交易日 15:00 收盘，21:00 晚报
对应当日 15:00 收盘。定时任务缺少截止时间时必须失败关闭，不能回退到实时 quote。
