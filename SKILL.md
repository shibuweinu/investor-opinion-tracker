...
3. 默认优先使用用户已登录的外置 Chrome/CDP，而不是内置浏览器；直接调用仓库内置的 `ExternalChromeXueqiuCollector`，不依赖另装雪球抓取 Skill。仅在外置浏览器不可用时再回退到宿主浏览器适配器。固定帖不决定分页停止；必须用最后一条非置顶普通帖判断边界并按帖子 ID 去重，遵守 QPS 和时间边界。
4. 保留原文链接和时间，区分本人观点、引用、转发、玩笑、事实转述与持仓披露。含糊内容不得作为正式观点或业绩统计依据。
5. 显式股票代码优先；行业映射必须注明推导。A股行情直接使用仓库内置的 `TdxClient`，用户无需安装 `tdx-api` Skill；AKShare 用于 ETF、港股及 TDX 失败时的回退。证据按 A（公告/交易所）、B（权威行情/公司资料）、C（可信媒体/研报）、D（未经核验）分级。
