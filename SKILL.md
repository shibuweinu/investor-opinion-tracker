...
1. Agent 安装时运行 `opinion-tracker init --no-interactive`。未绑定远端配置时必须让用户选择 `restore`、`create` 或 `skip`；使用 `config-connect` 或 `--config-repo` 只连接并预览，不能自动导入。新工作区不得复用历史对话中的博主或示例 URL，也不得把“继续”或安装许可当成任务确认。
